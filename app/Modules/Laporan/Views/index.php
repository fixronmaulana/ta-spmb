...
<div class="space-y-6">
    ...
    <!-- ══════════════════════════════════════════════════════════
         3 & 4. TOMBOL EXPORT + TABS
         Digabung dalam satu x-data Alpine.js agar tombol ekspor
         otomatis menyesuaikan tab yang aktif (via query string ?tab=...)
    ══════════════════════════════════════════════════════════ -->
    <div x-data="{ tab: 'jurusan' }" class="space-y-4">

        <!-- Baris atas: Tab pills (kiri) + tombol ekspor (kanan) -->
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            <!-- Tombol Export — URL berubah sesuai tab aktif -->
            <div class="flex flex-wrap gap-2 flex-shrink-0">

                <!-- Export PDF -->
                <a :href="'<?= base_url('kepala-sekolah/laporan/ekspor-pdf') ?>?tab=' + tab"
                    target="_blank"
                    ...
                    <span x-text="'Export PDF (' + {'jurusan':'Per Jurusan','gelombang':'Per Gelombang','demografi':'Demografi','tren':'Tren'}[tab] + ')'">Export PDF</span>
                </a>

                <!-- Export Excel -->
                <a :href="'<?= base_url('kepala-sekolah/laporan/ekspor-excel') ?>?tab=' + tab"
                    class="inline-flex items-center gap-2 px-3 py-1.5 border border-gray-300 rounded-xl text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors">
                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3" />
                    </svg>
                    <span x-text="'Export Excel (' + {'jurusan':'Per Jurusan','gelombang':'Per Gelombang','demografi':'Demografi','tren':'Tren'}[tab] + ')'">Export Excel</span>
                </a>
        ...
        <div x-show="tab === 'tren'" x-transition class="space-y-6">
            ...
            <div class="bg-white rounded-2xl border border-gray-200">
                ...
                <div class="p-5">
                    ...
                    <?php else: ?>
                        <div class="relative w-full" style="height:min(260px,55vw)">
                            <div class="relative w-full" style="height:min(240px,55vw)"><canvas id="chartTren" height="240"></div>
                        </div></canvas>
                    <?php endif; ?>
                </div>
            </div>
