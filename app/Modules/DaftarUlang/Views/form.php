 ...
 *
 * FIX #1: Dropdown kelas hanya dari jurusan_diterima_id (sudah benar di controller),
 *         label dropdown menampilkan nama jurusan diterima agar siswa tahu kelas apa yg tampil.
 * FIX #3: Variabel $pending (ganti $existing) — form hanya tampil jika tidak ada PENDING.
 *         Tidak ada "bukti sebelumnya tersimpan" karena setiap submit adalah row baru.
 */

// FIX #3: Pakai $pending (dari controller baru) — bukan $existing
...
// FIX #3: $pending = pengajuan yang sedang menunggu konfirmasi admin
// Jika ada $pending → form ditutup, siswa harus tunggu dulu
$adaPending = ! empty($pending);

// Timeline state
$duStatus = $pending->status ?? null; // null | pending | ditolak (dikonfirmasi sudah redirect sebelumnya)
?>
...
<div class="space-y-6 max-w-4xl mx-auto">
    ...
    <!-- ══ FIX #3: BANNER ADA PENGAJUAN PENDING ═══════════════════════════ -->
    <?php if ($adaPending): ?>
        <div class="flex items-start gap-3 p-4 rounded-2xl"
            ...
            <div>
                ...
                <p class="text-xs mt-0.5" style="color:hsl(38,60%,38%);">
                    ...
                    Formulir baru tidak dapat diisi sampai pengajuan ini selesai diproses.
                </p>
                <a href="<?= base_url('dashboard/daftar-ulang/status') ?>"
                    class="inline-flex items-center gap-1 text-xs font-semibold mt-2"
    ...
    <div class="grid lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-2xl overflow-hidden"
            ...
            <?php if ($adaPending): ?>
                <!-- FIX #3: Form ditutup saat ada PENDING -->
                <div class="px-6 py-10 text-center">
                    <div class="w-14 h-14 rounded-full flex items-center justify-center mx-auto mb-4"
                        style="background:hsl(38,92%,50%,.15);">
                        <svg class="w-7 h-7" style="color:hsl(38,60%,38%);" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                            <circle cx="12" cy="12" r="10" />
                            <polyline points="12 6 12 12 16 14" />
                        </svg>
                    </div>
                    <p class="font-semibold mb-1" style="color:hsl(220,54%,15%);">Pengajuan Menunggu Konfirmasi</p>
                    <p class="text-sm" style="color:hsl(220,15%,55%);">
                        Formulir baru akan tersedia setelah admin memproses pengajuan Anda saat ini.
                    </p>
                    <a href="<?= base_url('dashboard/daftar-ulang/status') ?>"
                        class="inline-flex items-center gap-2 px-5 py-2 text-sm font-semibold text-white rounded-xl mt-4 transition"
                        style="background:hsl(220,54%,20%);">
                        Lihat Status Pengajuan
                    </a>
                </div>
            <?php else: ?>
                <!-- FIX #3: Form aktif hanya jika tidak ada PENDING -->
                <form action="<?= base_url('dashboard/daftar-ulang') ?>" method="POST" enctype="multipart/form-data"
                    ...
                    <!-- FIX #1: Pilih Kelas — label menjelaskan kelas dari jurusan diterima -->
                    <?php if (! empty($kelasList)): ?>
                        <div>
                            ...
                            <!-- FIX #1: Hint jurusan agar siswa tahu kelas yang tampil dari jurusan mana -->
                            <p class="text-xs mb-2 flex items-center gap-1" style="color:hsl(220,15%,58%);">
                                <svg class="w-3.5 h-3.5 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    <circle cx="12" cy="12" r="10" />
                                    <line x1="12" y1="8" x2="12" y2="12" />
                                    <line x1="12" y1="16" x2="12.01" y2="16" />
                                </svg>
                                Menampilkan kelas dari jurusan yang Anda terima:
                                <strong style="color:hsl(220,54%,20%);"><?= esc($jurusanNama) ?></strong>
                            </p>
                    ...
                    <?php elseif ($pendaftaran->jurusan_diterima_id): ?>
                        <!-- FIX #1: Belum ada kelas terdaftar untuk jurusan diterima -->
                        <div class="p-3 rounded-xl" style="background:hsl(38,92%,50%,.08);border:1px solid hsl(38,92%,50%,.25);">
                            <p class="text-xs" style="color:hsl(38,55%,35%);">
                                Belum ada kelas aktif untuk jurusan
                                <strong><?= esc($jurusanNama) ?></strong>.
                                Penempatan kelas akan ditetapkan oleh admin.
                            </p>
                        </div>
                    <?php endif; ?>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="nominal_pembayaran" required
                                placeholder="0"
                                value="<?= number_format($nominalDaftarUlang, 0, ',', '.') ?>"
                    ...
                    <div>
                        ...
                        <textarea name="catatan_siswa" rows="3"
                            ...
                            style="border-color:hsl(220,20%,85%);color:hsl(220,54%,15%);"
                            @focus="this.style.borderColor='hsl(220,54%,20%)';this.style.boxShadow='0 0 0 3px hsl(220 54% 20%/.1)'"
                            @blur="this.style.borderColor='hsl(220,20%,85%)';this.style.boxShadow='none'"></textarea>
                    </div>
    ...
    <div class="bg-white rounded-2xl p-6"
        ...
        if ($duStatus === 'pending')      $stepState = 3;
        if ($duStatus === 'dikonfirmasi') $stepState = 5;
        if ($duStatus === 'ditolak')      $stepState = 2;
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-2 relative">
                <?php foreach ($steps as $step):
                    ...
                    if ($duStatus === 'ditolak' && $num === 2) {
                        ...
                        $badgeLabel = 'Ditolak';
                        $icon       = '<svg class="w-4 h-4" fill="none" stroke="white" stroke-width="2.5" viewBox="0 0 24 24"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>';
                    } elseif ($isDone) {
